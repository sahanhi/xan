    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        btn = [
            [
                InlineKeyboardButton(
                    "𝙊𝙐𝙍 𝙈𝘼𝙄𝙉 𝘾𝙃𝘼𝙉𝙉𝙀𝙇", url=invite_link.invite_link
                )
            ]
        ]
        ...
        if message.command[1] != "subscribe":
            try:
                ...
                btn.append([InlineKeyboardButton("𝙏𝙍𝙔 𝘼𝙂𝘼𝙄𝙉", callback_data=f"{pre}#{file_id}")])
            except IndexError:
                btn.append([InlineKeyboardButton("𝙏𝙍𝙔 𝘼𝙂𝘼𝙄𝙉", url=f"https://t.me/{temp.U_NAME}/{message.command[1]}")])
